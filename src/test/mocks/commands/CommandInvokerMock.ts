...
import { Arguments } from './ArgumentsMock';
import { HelpCommand } from './HelpMock';
import { RiotCommand } from './RiotMock';
import { KanyeCommand } from './KanyeMock';
import { DefaultCommand } from './DefaultCommandMock';
import { PingCommand } from './PingCommandMock';
import { PomodoroCommand } from './PomodoroCommandMock';
...
export class CommandInvoker {
    ...
    setCommand(message: Message): void | null {
        ...
        switch (commandType) {
            ...
            case CommandType.riot: {
                this.command = new RiotCommand(args.parameter1!);
                break;
            }
            case CommandType.kanye: {
                this.command = new KanyeCommand();
                break;
            }
            case CommandType.pomodoro: {
                this.command = new PomodoroCommand(args.parameter1!, args.parameter2!);
                break;
            }
    ...
    executeCommand() : string {
        return (this.command == undefined) ? 'invalid command' : this.command.execute();
    }
